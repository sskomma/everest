 ...
 * but they’are not fully constructed by other words. Former one has an extra x letter, and latter
 * is an individual word by itself not a compound word.
 *
...
public class LongestCompoundWord {
  ...
  public String findLongestCompountWord(String[] words) {
    ...
    for (String word : words) {
      List<String> prefix = trie.insertAndGetPrefixes(word);
      for (String pre : prefix) {
        pairs.add(new Pair(word, word.substring(pre.length())));
      }
    }
    while (!pairs.isEmpty()) {
      Pair p = pairs.remove();
